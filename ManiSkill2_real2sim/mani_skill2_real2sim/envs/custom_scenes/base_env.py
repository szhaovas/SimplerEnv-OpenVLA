...
from mani_skill2_real2sim.agents.robots.jaco import Jaco
...
class CustomSceneEnv(BaseEnv):
        ...
        **kwargs,
        ...
        if rgb_overlay_path is not None:
            ...
            self.rgb_overlay_img = (
                cv2.cvtColor(cv2.imread(rgb_overlay_path), cv2.COLOR_BGR2RGB)
                / 255
            )  # (H, W, 3); float32
        else:
    ...
    def _load_arena_helper(self, add_collision=True):
        ...
        if self.scene_name is None:
            ...
            scene_path = str(
                self.scene_root / "stages" / f"{self.scene_name}.glb"
            )
        ...
        if self.scene_offset is None:
            if "google_robot_static" in self.robot_uid:
                ...
                # jaco env uses the scene google_pick_coke_can_1_v4.glb
        ...
        if self.scene_name is not None:
            ...
            if "modern_bedroom" in self.scene_name:
                scene_offset = np.array([-1.6616, -3.0337, 0.0])
                scene_pose = (
                    sapien.Pose([0.178, -2.235, 1.669], [0.007, 0, 0, -1])
                    * scene_pose
                )
    ...
    def _load_agent(self):
        agent_cls: Type[GoogleRobotStaticBase] = self.SUPPORTED_ROBOTS[
            self.robot_uid
        ]
    ...
    def _initialize_agent(self):
        ...
        if "google_robot_static" in self.robot_uid:
            ...
            if self.robot_uid in [
                "widowx",
                "widowx_bridge_dataset_camera_setup",
            ]:
                qpos = np.array(
                    [
                        -0.01840777,
            ...
            qpos = np.array([-1.5, 2.93, 1, -2.09, 1.44, 1.32, 0.02, 0.02])
            # qpos = np.array([-1.304, 4.003, 1.381, -2.234, 1.743, 1.908, 0.258, 0.258])
            robot_init_height = (
                0.87 + 0.017
            )  # base height + ground offset in default scene
            robot_init_rot_quat = [0, 0, 0, 1]
        ...
        if (
            robot_init_xy := self.robot_init_options.get("init_xy", None)
        ) is not None:
            robot_init_xyz = [
                robot_init_xy[0],
                robot_init_xy[1],
        ...
        self.agent.robot.set_pose(
            sapien.Pose(robot_init_xyz, robot_init_rot_quat)
        )
